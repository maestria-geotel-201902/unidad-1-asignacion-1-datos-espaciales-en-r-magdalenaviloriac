...
## Reading layer `PROVCenso2010' from data source `/home/magda/unidad-1-asignacion-1-datos-espaciales-en-r-magdalenaviloriac/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '20', 'PROV'])
```
...
## 20   20  03    SAMANÁ   0320 MULTIPOLYGON (((470705.9 21...
plot(miprov)
```
...
``` r
miprov %>% dplyr::filter(PROV %in% '20')
## Simple feature collection with 1 feature and 4 fields
## geometry type:  MULTIPOLYGON
## dimension:      XY
## bbox:           xmin: 418909.8 ymin: 2100080 xmax: 484340.3 ymax: 2141114
...
prov %>% dplyr::filter(PROV %in% '20') %>% dplyr::select(areaenm2) %>% plot
```
...
``` r
prov %>% dplyr::filter(PROV %in% c('20', '24', '23')) %>% dplyr::select(areaenm2) %>% plot
```
...
## Reading layer `MUNCenso2010' from data source `/home/magda/unidad-1-asignacion-1-datos-espaciales-en-r-magdalenaviloriac/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '20')
mimun %>% plot
```
